...
function runOpt(d, pfail, δ, repeats, heavy_tailed::Bool;
				problem=:phase_retrieval, ϵ_stop=1e-17)
	...
	if problem == :phase_retrieval
		D = heavy_tailed ? LogNormal(0, 1) : Normal(0, 1)
		prob = pLib.genProb(D, d, pfail=pfail)
	else
		L = heavy_tailed ? LogNormal(0, 1) : Normal(0, 1)
		R = heavy_tailed ? LogNormal(0, 1) : Normal(0, 1)
		prob = pLib.genProb(L, R, d, pfail=pfail)
	end
...
s = ArgParseSettings(description="""
	Compare stochastic subgradient and proximal point algorithms on phase
	retrieval under the streaming, fixed budget setting, where each sample
	is drawn from an infinite pool.""")
@add_arg_table s begin
	...
	"--heavy_tailed"
		help = "Set to use a log-normal distribution to generate the measurement vectors"
		action = :store_true
end
...
repeats, heavy_tailed = parsed["repeats"], parsed["heavy_tailed"]
runOpt(d, p, δ, repeats, heavy_tailed, problem=Symbol(probType), ϵ_stop=1e-16)
