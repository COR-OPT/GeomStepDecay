...
module RobustPR
	...
	struct PrProb
		A :: Union{Array{<:Number, 2}, Distribution}
		y :: Array{<:Number, 1}
		x :: Array{<:Number, 1}
		pfail :: Float64
	end
	...
	from a phase retrieval problem.
	"""
	function sampleVecs(prob::PrProb; samples=1)
		if isa(prob.A, Array)
			indices = rand(1:size(prob.A)[1], samples)
			return prob.A[indices, :], prob.y[indices]
		else
			d = length(prob.x); p = prob.pfail
			measVecs = rand(prob.A, samples, d)
			yMeas = (measVecs * prob.x).^2; replace!(_bigCorr(p), yMeas)
			return measVecs, yMeas
		end
	end


	#= stochastic subgradient =#
	function _subgrad(prob, xCurr, ai, b)
	   w = ai' * xCurr; res = sign(w^2 - b)
	   return 2 * ai * (res * w)
	end
